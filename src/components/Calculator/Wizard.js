...
export default class StepZilla extends Component {
  ...
  jumpToStep(evt) {
    ...
    if (evt.target == undefined) {
      ...
      const movingBack = target.value < this.state.compState; // are we trying to move back or front?
      ...
      this.abstractStepMoveAllowedToPromise(movingBack)
          ...
          if (proceed) {
            if (!movingBack) {
              ...
              passThroughStepsNotValid = this.props.steps
                .reduce((a, c, i) => {
                  if (i >= this.state.compState && i < target.value) {
                    a.push(c.validated);
                  }
                  return a;
                }, [])
        ...
        .then(() => {
          ...
          if (proceed && !passThroughStepsNotValid) {
            if (target.value === (this.props.steps.length - 1) &&
              this.state.compState === (this.props.steps.length - 1)) {
                this.setNavState(this.props.steps.length);
            }
            else {
              this.setNavState(target.value);
            }
          }
        })
        .catch(e => {
